...
export default function MyAccountPage() {
  ...
  const handleFormSubmit = (event) => {
    ...
    if (isServiceProviderRole) {
      ...
      if (customerToken && !userData?.userVerificationStatus?.verifiedLevel) submitManagerEscortData(userData.user, true, userData.userId);
      requestReviewForApproval(userData.requestType);
    } else {
      // update User data
      if (customerToken) submitData(userData.user, true);
    };
  };
  ...
  const submitManagerEscortData = async (formData, showSuccessMessage, escortId) => {
    console.log('formData, showSuccessMessage', formData, showSuccessMessage );
    
    const memberToken = process.env.NEXT_PUBLIC_API_TOKEN_MEMBER;
    if (!memberToken || !escortId || !formData) return;
    const id = userData?.userId || escortId;
    console.log('escortId submitManagerEscortData', escortId);
    
    useFetchData(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/members/${escortId}`, {
      method: 'PUT',
      body: JSON.stringify(formData),
      headers: {
        'Content-type': 'application/json',
  ...
  const handleUpdateUser = (field, value, isProfileEditPage) => {
    ...
    if (!requiresApproval && !isProfileEditPage) {
      ...
      if (isServiceProviderRole) {
        ...
        submitManagerEscortData(updatedUserData, isVerified, userData.userId);
      };
    };
  };
console.log('userData.userId', userData.userId);
  ...
  const updateUserPendingOvverridesAndSubmitUserData = (fieldsUpdatedData) => {
    ...
    userData.role === MANAGER.type ? submitManagerEscortData(updatedUserData, false, userData.userId) : submitData(updatedUserData);
    const isVerified = userData?.userVerificationStatus?.verifiedLevel ? true : false;
  ...
  const applyAdminApprovalFieldsChangesForVerifiedProvider = async () => {
    ...
    userData?.role === MANAGER.type ? submitManagerEscortData(userWithUpdatedPendingData, false, userData.userId) : submitData(userWithUpdatedPendingData);
    // sent a review link for admin
        ...
        <form onSubmit={(e) => handleFormSubmit(e)}>
          ...
          {userData?.user && userData?.role === CLIENT.type &&
            <>
              ...
              />
              {['dataChangeWithoutRequest', 'errorInfo'].map(
                (key) =>
                  notificationState?.[key]?.show && (
                    <Popup
                      key={key}
